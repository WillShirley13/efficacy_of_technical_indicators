...
def get_stock_data() -> dict[str, pd.DataFrame]:
    ...
    for stock in stock_data.keys():
        ...
        if required_cols - actual_cols:
            raise ValueError(
                f"{stock} Missing required columns, has {actual_cols}, needs {required_cols}"
            )
...
def logical_checks(stock_data: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    ...
    for stock_name, data in stock_data.items():
        ...
        if high_lt_low.any():
            ...
            print(
                df.loc[high_lt_low, ["Open", "High", "Low", "Close", "Volume"]].head(5)
            )

        # Validate prices are non-negative
        price_lt_zero: pd.Series = (df[["Close", "High", "Low", "Open"]] < 0).any(
            axis=1
        )
        if price_lt_zero.any():
            print(
                f"[{stock_name}] Negative price violations: {int(price_lt_zero.sum())}"
            )
            print(
                df.loc[price_lt_zero, ["Open", "High", "Low", "Close", "Volume"]].head(
                    5
                )
            )
        ...
        if vol_le_zero.any():
            ...
            print(
                df.loc[vol_le_zero, ["Open", "High", "Low", "Close", "Volume"]].head(5)
            )
        ...
        if drop_mask.any():
            ...
            print(
                f"[{stock_name}] Dropped assertion-violating rows: {int(drop_mask.sum())}"
            )
        ...
        if dup_idx_all.any():
            ...
            print(
                f"[{stock_name}] Duplicate index (date) rows: {dup_idx_count} (keeping last)"
            )
